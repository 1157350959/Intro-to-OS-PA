...
int count_page_faults_fifo(struct PTE page_table[TABLEMAX], int table_cnt, int reference_string[REFERENCEMAX], int reference_cnt, int frame_pool[POOLMAX], int frame_cnt)
{
    ...
    for(int i = 0, time_stamp = 1; i < reference_cnt; i++, time_stamp++)
    {
        if(page_table[reference_string[i]].is_valid == true)
        {
            ...
            printf("page: %d, 1st case. Current faults: %d\n",reference_string[i],total_page_faults);
        }
        else
        {
            if(frame_cnt > 0)
            {
                ...
                table_cnt = (reference_string[i] >= table_cnt)? table_cnt + 1 : table_cnt;
                total_page_faults++;
                printf("page: %d, 2nd case. Current faults: %d\n",reference_string[i],total_page_faults);
            }
            else
            {
                int min_arrival_time = -1;
                ...
                page_table[min_arrival_time_idx] = replaced_pte;
                total_page_faults++;
                printf("page: %d, 3rd case. Current faults: %d\n",reference_string[i],total_page_faults);
            }
        }
    }
...
//     int frame_cnt = 3;
//     int faults = 0;
//     faults = count_page_faults_fifo(page_table,tabel_cnt,reference_string,reference_cnt,frame_pool,frame_cnt);
//     printf("%d",faults);
//     return 0;
// }