...
struct MEMORY_BLOCK first_fit_allocate(int request_size, struct MEMORY_BLOCK memory_map[MAPMAX], int * map_cnt, int process_id)
{
    ...
    int lowest_starting_address_idx = -1;
    for(int i = 0; i < *map_cnt; i++)
    {
        if(memory_map[i].segment_size >= request_size && memory_map[i].process_id == 0)
        {
            lowest_starting_address = (lowest_starting_address == -1)? memory_map[i].start_address : lowest_starting_address;
            lowest_starting_address_idx = (lowest_starting_address_idx == -1)? i : lowest_starting_address_idx;
            ...
            lowest_starting_address_idx = (memory_map[i].start_address < lowest_starting_address)? i : lowest_starting_address_idx;
        }
    }
    if(lowest_starting_address != -1)
    {
        if(memory_map[lowest_starting_address_idx].segment_size == request_size)
        {
            memory_map[lowest_starting_address_idx].process_id = process_id;
            return memory_map[lowest_starting_address_idx];
        }
        else
        {
            struct MEMORY_BLOCK new_free_block = {memory_map[lowest_starting_address_idx].start_address + request_size,
                                                memory_map[lowest_starting_address_idx].end_address,
                                                memory_map[lowest_starting_address_idx].segment_size - request_size,
                                                0};
            memory_map[*map_cnt] = new_free_block;
            memory_map[lowest_starting_address_idx].end_address = memory_map[lowest_starting_address_idx].start_address + request_size - 1;
            memory_map[lowest_starting_address_idx].segment_size = request_size;
            memory_map[lowest_starting_address_idx].process_id = process_id;
            *map_cnt += 1;
            return memory_map[lowest_starting_address_idx];
